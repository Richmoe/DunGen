...
class MapGenerator {
    ...
    init(map: Map) {
      ...
      func doMoves(passageMoves: [TileCode], fromPassage: Passage, fromPt: MapPoint)  {
          ...
          for move in passageMoves {
            ...
            if (map.offScreen(point: curPoint)) {
              ...
              //Check Overlap
              if (map.mapBlocks[curPoint.row][curPoint.col].tileCode != TileCode.null) {
                  //Here's where we check for what to do
                  //print("Overlap! at \(curPoint)")
                  
                  
                  //Check to see what the block has in my direction:
                  let code = map.mapBlocks[curPoint.row][curPoint.col].getWallCode(wallDir: curHeading.direction.opposite())
                  //print ("the wall code at the collision is: \(code)")
                  
                  //For now just mirror that:
                map.mapBlocks[prevPt.row][prevPt.col].addCode(codeDir: curHeading.direction, code: code)
                  
                  return
              } else {
                map.mapBlocks[curPoint.row][curPoint.col] = MapBlock(tileCode: move, fromPassage: curHeading)
              }
      ...
      func generateStart(at: MapPoint) {
          ...
          //plot entrance:
        map.mapBlocks[mapSpot.row][mapSpot.col] = MapBlock(tileCode: .stairsUp, heading: .north)
          ...
          case 9, 10:
              ...
              //hall of stairs
            map.mapBlocks[mapSpot.row][mapSpot.col] = MapBlock(tileCode: .passage, fromPassage: Passage(type: .hallway, direction: .north))
              
              //crossroad
              mapSpot += MapPoint(row: 1, col: 0)
            map.mapBlocks[mapSpot.row][mapSpot.col] = MapBlock(tileCode: .turnRight, fromPassage: Passage(type: .hallway, direction: .north))
              
              //Hall to right
              var passage = Passage(type: .hallway, direction: .east)
            map.mapBlocks[mapSpot.row][mapSpot.col + 1] = MapBlock(tileCode: .passage, fromPassage: passage)
              addToQueue(from: MapPoint(row: mapSpot.row, col: mapSpot.col + 1), passage: passage)
              
              //Hall to left
              passage = Passage(type: .hallway, direction: .west)
              map.mapBlocks[mapSpot.row][mapSpot.col].addCode(exit: Passage(type: .hallway, direction: .west))
              map.mapBlocks[mapSpot.row][mapSpot.col - 1] = MapBlock(tileCode: .passage, fromPassage: passage)
              addToQueue(from: MapPoint(row: mapSpot.row, col: mapSpot.col - 1), passage: passage)
              ...
              if (roll == 10) {
                  ...
                  map.mapBlocks[mapSpot.row][mapSpot.col].addCode(exit: Passage(type: .hallway, direction: .north))
                  map.mapBlocks[mapSpot.row + 1][mapSpot.col] = MapBlock(tileCode: .passage, fromPassage: passage)
      ...
      func fixUpMap() {
          ...
          for r in (0..<map.mapBlocks.count) {
              for c in 0..<map.mapBlocks[r].count  {
                  ...
                  if (r < (map.mapHeight - 1)) {
                      ...
                      codeThis = map.mapBlocks[r][c].getWallCode(wallDir: .north)
                      codeThat = map.mapBlocks[r+1][c].getWallCode(wallDir: .south)
                      ...
                      if (codeThis != codeThat && String([codeThis,codeThat]) != "0W" && String([codeThis,codeThat]) != "W0" && codeThis != "+" && codeThat != "+" && codeThis != "-" && codeThat != "-")  {
                          ...
                          if (["DP", "PD"].contains(String([codeThis, codeThat]))) {
                              map.mapBlocks[r+1][c].addCode(codeDir: .south, code: "D")
                              map.mapBlocks[r][c].addCode(codeDir: .north, code: "D")
                          } else                         if (["SP", "PS"].contains(String([codeThis, codeThat]))) {
                              map.mapBlocks[r+1][c].addCode(codeDir: .south, code: "S")
                              map.mapBlocks[r][c].addCode(codeDir: .north, code: "S")
                          } else if (codeThis == "0") {
                              map.mapBlocks[r+1][c].addWall(wallDir: .south)
                          } else if (codeThat == "0") {
                              map.mapBlocks[r][c].addWall(wallDir: .north)
                          } else {
                              //pick one to be authority:
                              if (MapGenRand.sharedInstance.getRand(to: 2) == 2) {
                                  map.mapBlocks[r+1][c].addCode(codeDir: .south, code: codeThis)
                                  
                              } else  {
                                  map.mapBlocks[r][c].addCode(codeDir: .north, code: codeThat)
                              }
                          }
                  ...
                  if (c < (map.mapWidth - 1)) {
                      ...
                      codeThis = map.mapBlocks[r][c].getWallCode(wallDir: .east)
                      codeThat = map.mapBlocks[r][c+1].getWallCode(wallDir: .west)
                      if (codeThis != codeThat && String([codeThis,codeThat]) != "0W" && String([codeThis,codeThat]) != "W0" && codeThis != "+" && codeThat != "+" && codeThis != "-" && codeThat != "-") {
                          //print ("Fixing up EW at \(r), \(c): west: \(mapBlocks[r][c].wallString), east: \(mapBlocks[r][c+1].wallString)")
                          if (["DP", "PD"].contains(String([codeThis, codeThat]))) {
                              map.mapBlocks[r][c+1].addCode(codeDir: .west, code: "D")
                              map.mapBlocks[r][c].addCode(codeDir: .east, code: "D")
                          } else if (["SP", "PS"].contains(String([codeThis, codeThat]))) {
                              map.mapBlocks[r][c+1].addCode(codeDir: .west, code: "S")
                              map.mapBlocks[r][c].addCode(codeDir: .east, code: "S")
                          } else if (codeThis == "0" || map.mapBlocks[r][c].tileCode == TileCode.stairsUp || map.mapBlocks[r][c].tileCode == TileCode.stairsDown) {
                              map.mapBlocks[r][c+1].addWall(wallDir: .west)
                          } else if (codeThat == "0" || map.mapBlocks[r][c].tileCode == TileCode.stairsUp || map.mapBlocks[r][c].tileCode == TileCode.stairsDown) {
                              map.mapBlocks[r][c].addWall(wallDir: .east)
                          } else {
                              //pick one to be authority:
                              if (MapGenRand.sharedInstance.getRand(to: 2) == 2) {
                                  map.mapBlocks[r][c+1].addCode(codeDir: .west, code: codeThis)
                                  
                              } else  {
                                  map.mapBlocks[r][c].addCode(codeDir: .east, code: codeThat)
                              }
                          }
                          //print ("---after \(r), \(c): this(lower): \(mapBlocks[r][c].wallString), upper: \(mapBlocks[r+1][c].wallString)")
          ...
          for r in (0..<map.mapBlocks.count) {
              for c in 0..<map.mapBlocks[r].count  {
                  ...
                  if (map.mapBlocks[r][c].tileCode != .stairsUp && map.mapBlocks[r][c].tileCode != .stairsDown) {
                      //Now check for and walk back dead ends:
                      //let block = mapBlocks[r][c].getBlock()
                      let wallCount = map.mapBlocks[r][c].wallString.filter { $0 == "W" }.count // case-sensitive
      ...
      func trimDeadEnd(at: MapPoint) {
          ...
          while (!map.offScreen(point: curPt)) {
              ...
              if (wallCount == 3) {
                  ...
                  //wipe
                  map.mapBlocks[curPt.row][curPt.col] = MapBlock()
      ...
      func doesOverlap(room: Room) -> Bool {
          for r in 0...(room.height - 1) {
              ...
              for c in 0...(room.width - 1) {
                  ...
                  if (map.mapBlocks[rm][cm].tileCode != TileCode.null) {
                      //if (floor[rm][cm] > 0) {
